...
colnames(whole_table) <- c("pacific_Time_Span", "pacific_Max_Wind", "date",
...
weather_time_span_data <- weather_data %>%
  mutate(time_span = as.numeric(difftime(endT, startT, tz = "UTC", units = "mins"))) %>%
  filter(time_span != "NA") 
...
server <- function(input, output) {
  output$severe_weather_state_map <- renderPlotly({
    ...
    my_plot <- ggplot(state_shape) +
      ...
      labs(title = "Number of Severe Weather Per State in U.S.",
  ...
  output$hurricane <- renderPlot({
    if (input$y_axis_input == "Atlantic") {
      ggplot(data = whole_table) +
        geom_point(mapping = aes(x = date, y = atlantic_Time_Span, 
                                 color = atlantic_Max_Wind)) +
        labs(title = "Time Span of Hurricane from 1949 to 2015", x = "Year", 
             y = "Atlantic Time Span", color = "Atlantic Max Wind")
    } else {
      ggplot(data = whole_table) +
        geom_point(mapping = aes(x = date, y = pacific_Time_Span, 
  ...
  output$plot <- renderPlot({
    p <- ggplot(data = weather_time_span,
    ...
    if (input$smooth) {
      p <- p + geom_boxplot(data = weather_time_span_data,
